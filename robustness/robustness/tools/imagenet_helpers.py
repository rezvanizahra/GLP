...
class Node():
    ...
    def add_1st_child(self, child):
        ...
        Args:
            child (Node) : Node object for child
        """
        self.child_wnid.append(child.wnid)
    
    def __str__(self):
        return f'Name: ({self.name}), ImageNet Class: ({self.class_num}), \
        1st-Child: ({self.child_wnid}) , Descendants: ({self.descendant_count_in})'
...
class ImageNetHierarchy():
    ...
    def __init__(self, ds_path, ds_info_path):
        ...
        with open(os.path.join(ds_info_path, 'wordnet.is_a.txt'), 'r') as f:
            for line in f.readlines():
                ...
                parentNode.add_1st_child(childNode)
        #         if parent_wnid == 'n00027807':
        #           print(parentNode.name , child_wnid, childNode.name)
        #           entity_child.append(child_wnid)
        # print(entity_child)
        #         if parent_wnid not in all_parents: 
        #             all_parents[parent_wnid]=1
        #         else: 
        #             all_parents[parent_wnid]+=1
        # print(all_parents)
    ...
    def get_superclasses(self, n_superclasses, 
        ...
        class_ranges, label_map = self.get_subclasses(superclass_wnid, 
                                    balanced=balanced)
        names = []
        for s in superclass_wnid:
            names.append(self.wnid_to_name[s])
        return superclass_wnid, class_ranges, label_map, names
